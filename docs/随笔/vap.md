...
为什么会去阅读 vap 源码？因为在使用的过程中，发现有一个很奇怪的bug，当svga资源文件为60帧时，会出现无法播放视频只有音频的情况。尽管通过设置accurate为true启用精准模式，可以解决这个问题，但还是比较好奇，所以需要去阅读源码，了解其背后的原理。
...
至此，我们完成了 video.ts 的源码阅读。在这过程中，其实我们也发现了为什么60帧fps无法播放，因为源码写死了60，当svga资源fps为60时，60/60=1，index % 1 永远为0，!(0)为true，会每帧都执行cb()，导致程序错误，没有调用play()。
...
// 修改后
const frameSkip = fps > 0 ? Math.round(60 / fps) : 1;
if (!(index % frameSkip)) {
  return cb();
}
```