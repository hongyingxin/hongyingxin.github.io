...
由于前端发布上线基本都采用了混淆压缩的方式，导致上线代码报错了，只能看到错误信息，很难定位到具体是哪行代码出现的问题。

这个问题采用`sourcemap`的方式可以解决。

`sourcemap`是一个映射文件，它建立了压缩/混淆后的代码与原始源代码之间的对应关系，主要是为了方便错误调试，定位问题代码所在行以及性能分析工具以及Sentry等第三方错误监控工具。

因为我们采用了`vite`作为构建工具，所以需要使用`@sentry/vite-plugin`来上传 sourcemap。
...
sentryVitePlugin({
  ...
  sourcemaps: {
    filesToDeleteAfterUpload: ['build_v/**/*.js.map'], //上传后删除sourcemap文件
  },
}),
```
