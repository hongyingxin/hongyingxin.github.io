...
## React 如何实现类似于 Vue-router 提供的路由守卫

在 React 中实现类似 Vue-router 的路由守卫，常见的方式包括：

- 使用 useEffect 钩子
- 封装高阶组件
- 利用 React Router 的 <Outlet /> 组件进行中间件式守卫
- 在路由配置中实现守卫逻辑
- 通过全局状态管理工具（如 Redux 或 Context）进行守卫。

## React 19 有哪些新特性？

React 19 是一个**从‘手动优化’转向‘引擎优化’**的版本。它通过 React Compiler 解决了性能优化的心智负担，通过 Actions 规范化了异步状态处理，并通过 Server Components 和 use 等新特性，模糊了前端与后端的界限，让开发体验（DX）得到了极大提升。

## Redux Toolkit和Zustand的区别

RTK数据流式严格的单向数据流，Store结构通过Slice拆分，状态访问需要Provider包裹根组件。
Zustand可以直接修改数据，Store支持多个独立Store，状态访问不需要Provider，直接导入hook即可。
...
// 2. 注入 Store 并用 Provider 包裹 App (略)
// 3. 使用
function Counter() {
...
function Counter() {
  const { count, inc } = useStore() // 直接用，不用 Provider
...
在 React 的函数组件中，每次渲染都会重新执行函数。useState 的核心使命是通过闭包和链表结构，在函数执行之外开辟一块持久化空间，把状态“存”起来。

### Fiber上的Hook链表

每个组件在React内部都有一个对应的Fiber节点，这个节点上有一个属性叫memoizedState。
- 对于函数组件，memoizedState存放的是一个Hook对象的单向链表
- 每一个useState或useEffect都会按顺序生成一个Hook对象，并通过next指针依次连接

Hook对象的内部构造

```js
const hook = {
  memoizedState: null, // 存储 useState 的当前值
  baseState: null,     // 基础状态（用于处理高优先级中断后的合并）
  baseQueue: null,     // 基础更新队列
  queue: null,         // 存放待执行的更新动作（dispatch）
...
React内部会根据组件当前所处的阶段，切换不同的useState实现（通过ReactCurrentDispatcher切换）

**首次渲染**

1. 创建Hook节点：按照代码中useState出现的顺序，创建一个新的Hook对象
2. 初始化值：将传入的初始参数存入memoizedState
3. 绑定更新函数：创建一个dispatch函数（即setState），并预先绑定好当前Hook节点
4. 存入链表：把这个Hook挂在Fiber链表的末尾

**更新渲染**

1. 定位Hook：按照执行顺序，从Fiber的链表中依次取出对于的旧Hook节点
2. 计算新值：查看该Hook上的queue更新队列，如果有待处理的更新，则依次执行它们，计算出最终结果并更新memoizedState
3. 返回结果：返回[最新状态，之前的dispatch函数]

**伪代码**
```js
let workInProgressHook = null; // 当前处理到的 Hook 节点指针
let isMount = true;            // 是否是初次渲染

function useState(initialState) {
  let hook;

  if (isMount) {
    // 初次渲染：创建 Hook 对象并构建链表
    hook = { memoizedState: initialState, next: null };
