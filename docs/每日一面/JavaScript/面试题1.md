...
1. 使用`html2canvas`生成海报：`html2canvas`是一个将HTML元素渲染为Canvas的库，它可以将HTML元素转换为Canvas对象，然后使用Canvas的API将Canvas对象转换为图片。

2. 自己实现`DOM->canvas->image`即`html2canvas`的实现原理。具体步骤如下：

  - 递归遍历DOM树，通过 `window.getComputedStyle` 获取元素的样式
  - 构建层叠上下文，通过 `z-index` 、 `position` 、 `float` 等属性计算元素的渲染层级顺序
  - 转换坐标系，将DOM元素的和模型属性转换为Canvas的坐标系
  - 调用Canvas绘图API
  - 将Canvas对象转换为图片，使用Canvas的`toDataURL`方法或者`toBlob`方法将Canvas对象转换为图片

3. 使用SVG外联(ForeignObject)：

SVG外联(ForeignObject)是一种将SVG元素嵌入到HTML元素中的方式，它可以将SVG元素转换为HTML元素，然后使用HTML元素的API将HTML元素转换为图片。
