...
函数的定义有两种方式：函数声明和函数表达式。

函数声明会提升，可以在声明前调用；函数表达式不会提升，不能在声明前调用。

在同一作用域，函数声明在创建阶段优先于var声明的变量，但不会阻止后续赋值覆盖。

```javascript
console.log(typeof foo) // function
var foo = 1             // 运行期赋值覆盖函数绑定
function foo(){}
...
// 2.3IIFE立即执行函数，是函数声明后立即调用的函数表达式
(function(){
  console.log('-')
})()
...
// 4.Function构造函数
const foo = new Function('return "hello"')
```
...
4. 箭头函数调用模式，this指向包裹箭头函数的第一个函数对象的this
