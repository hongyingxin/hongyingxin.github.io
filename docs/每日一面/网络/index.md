...
- 1.0 使用**短连接**，每请求一个资源都需要经历一次TCP连接，三次握手和四次挥手，1.1 使用**持久连接**，可以复用TCP通道

- 1.0 请求是**串行**的，发送一个请求必须等待响应回来，1.1 引入**管道机制**，允许客户端同时发送多个请求。

- 1.0 主要是用`If-Modified-Since`和`Last-Modified`来判断资源是否修改，1.1 中引入了`ETag`和`Cache-Control`，

- 1.0 不支持**断点续传**，如果下载一个大文件连接中断只能从头开始， 1.1 引入了**Range**字段，允许只请求资源的某一部分

## HTTP 1.1 和 HTTP 2.0 的区别

- 1.1 中存在**队头阻塞**问题，一个TCP连接只能处理一个请求，虽然可以通过建立多个TCP连接来解决，但浏览器限制了同一个域名下的请求数量（Chrome下一般是限制六个连接），2.0 中引入了**多路复用**的技术，通过一个 TCP 连接就可以传输所有的请求数据。

- 1.1 是通过**文本**的方式传输数据，2.0 采用了**二进制**格式编码

- 1.1 请求头往往包含大量重复信息（Cookie、User-Agent等），2.0 中引入了**头部压缩**技术，通过HPACK算法对头部进行压缩，减少了头部的大小，提高了传输效率。

- 1.1 服务器必须等待客户端请求HTML里的资源（如CSS、JS）后才能返回，2.0 中引入了**服务器推送**技术，服务器可以主动向客户端推送资源，减少了往返时延，提高了传输效率。

- 1.1 无法原生支持资源优先级的调度，2.0 支持**资源优先级**，允许客户端为每个流设置权重。