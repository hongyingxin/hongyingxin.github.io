...
浏览器渲染原理是指浏览器如何将HTML、CSS、JavaScript代码转换成用户看到的页面的内部机制。这是一个复杂的多线程协作过程。
...
- 导航阶段：用户输入URL后，浏览器进行DNS查询找到服务器IP，然后建立TCP连接。这里的性能瓶颈主要是网络延迟。

- 响应阶段：服务器返回HTML文档，这时有个重要指标叫TTFB（首字节时间），直接影响用户感知的加载速度。

- 解析阶段：浏览器并行解析HTML构建DOM树，解析CSS构建CSSOM树。这里有个关键优化点事预加载扫描器，它会提前发现并请求资源。

- 渲染阶段：DOM和CSSOM合并成渲染树，然后进行布局计算、绘制像素，最后合成显示。

- 交互阶段：页面可以响应用户操作，这时候主要看TTI（可交互时间）指标。
...
- 关键渲染路径：CSS会阻塞渲染，JavaScript会阻塞解析。所以我们通常会把CSS放在head中尽早加载，对于非关键的JavaScript使用async或defer属性。

- 重排重绘：布局阶段计算元素位置和大小，如果后续修改了几何属性就会触发重排，成本很高。而只修改颜色等属性只会触发重绘，成本相对较低。

- 合成层优化：通过CSS的transform、opacity等属性可以创建合成层，利用GPU加速，避免影响主线程。"

**4. 实践经验总结**
>思路：结合具体优化经验，展现实战能力

基于这些原理，在项目中我会重点关注：首屏14KB的内容优化、关键资源的预加载、避免强制同步布局、合理使用Web Workers处理重计算。最终目标是让用户感受到快速的加载和流畅的交互。
