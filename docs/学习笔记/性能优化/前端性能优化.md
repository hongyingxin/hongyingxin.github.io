...
以及使用以下工具：Lighthouse、WebPageTest、Chrome DevTools 还有真实用户监控来收集这些指标。

## 3. 从第一性原理思考关键渲染路径

要解释性能优化，首先要理解将像素渲染到屏幕上的最小流程。思路简单明了：
...
1. **移除阻塞渲染的 CSS 和 JS**：将非关键的 CSS 移出关键渲染路径，非必要的脚本使用 defer 或 async 加载。
...
6. **减少不必要的重排和重绘**：合并 DOM 更新，避免 layout thrashing。
...
使用动态导入将大型 bundle 拆分为更小的模块。当脚本不需要在 HTML 解析之前执行时，使用 defer 属性避免阻塞页面加载。

React 中使用动态导入的示例（适用于 Webpack 或 Vite 等工具）
...
> FOIT 英语全称为 Flash of Invisible Text，是指“文字闪烁消失”的现象，这种情况常见于使用 @font-face 加载自定义字体时，浏览器默认会等待字体加载完成再进行渲染，导致用户在这段时间内看到一片空白区域，而不是后备字体或内容。
...
核心原则是保持主线程的响应性。任务执行时间超过 50 毫秒就被认为是“长任务”，可能导致界面响应迟缓。将大型任务拆分为更小的块，可以让浏览器在执行间隙处理用户输入。现在 React 的并发特性已经实现了自动拆分，但也可以手动使用 `requestIdleCallback` 或 `requestAnimationFrame` 来打断同步循环，实现类似效果。

防抖和节流是限制高频操作执行频率的常用手段。例如，在输入框中输入搜索词时，不应每按一次键就发送一次 API 请求。
...
它们可以保持 props 的引用相等性，从而让 React.memo 正常发挥作用，避免无意义的重复渲染。
...
对于包含成百上千的大型列表，通常需要使用虚拟列表技术来优化渲染性能。像 react-window 和 react-virtualized 等库，可以只渲染当前视口中可见的元素，显著减少 DOM 节点数量，从而提升初始渲染速度和滚动性能。

## 8. 构建系统与打包优化技巧

在构建阶段，合理配置打包工具不仅能减少最终传输的字节量，还能提升应用的启动速度和整体性能。以下是常见的构建优化策略：

- **Tree shaking**：移除未使用的导出，减小最终 bundle 大小。现代打包工具如 Webpack、Rollup、Vite 都支持。
- **代码压缩**：通过删除空格、缩短变量名等手段压缩 JS/CSS 文件，降低资源体积。
- **资源压缩**：在 CDN 层启用 Brotli 或 Gzip，对传输资源进行压缩，减少网络传输成本。
- **仅在开发环境使用 source map**：或在生产环境中通过身份验证保护 source map，避免泄漏源码结构。
...
- 在生产环境中持续追踪 Core Web Vitals 指标。
- 上线后监控是否出现性能回退。

真实用户监控能够展示你的网站在不同设备、网络和地区的真实使用表现。而合成监控则通过受控环境下的自动化测试，帮助提前发现性能回退问题。

> **真实用户监控**：指收集真实用户在实际使用过程中产生的性能数据，能够反映不同设备、网络和地区下的真实体验。
> **合成监控**：在受控环境中模拟用户访问，通过自动化脚本定期测试页面性能，适合做回归监测和预警。两者结合使用，可以全面掌握网站性能表现。

Performance API 提供了详细的时序数据，可以用它来衡量从导航开始到任务代码逻辑的执行时间。现代浏览器还支持 PerformanceObserver API，可以监听性能事件的发生。

Chrome DevTools 的 Performance 面板是识别性能瓶颈的重要工具。它能精准显示浏览器每一刻的行为：执行 JavaScript、布局计算、样式计算、渲染和绘制等。掌握性能分析工具的使用，能够清楚地揭示时间消耗在哪些环节，从而精准定位优化点。
...
这段代码展示了如何使用浏览器内置的 Performance API 和 PerformanceObserver API 来收集页面加载指标与 Core Web Vitals 数据，并通过 sendToAnalytics 上报。
