...
因为脚手架工具默认集成 Eslint，所以不需要安装。这里需要注意的一点是，如果是旧项目的话，Eslint 的配置文件不一样，自 ESlint 9.0 开始默认使用 `eslint.config.js` 文件，旧版本使用 `.eslintrc` 文件不再自动识别。

跟 Prettier 一样，Eslint 需要创建 `.eslintignore` 文件，添加忽略的文件。
